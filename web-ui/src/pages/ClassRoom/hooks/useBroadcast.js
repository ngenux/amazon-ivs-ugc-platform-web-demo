...
const { create, BASIC_FULL_HD_LANDSCAPE, StreamType } =
  window.IVSBroadcastClient;

const layouts = {
  1: { 1: { height: 700, width: 1260, x: 10, y: 10 } },
  2: {
    1: { height: 700, width: 625, x: 10, y: 10 },
    2: { height: 700, width: 625, x: 645, y: 10 }
  },
  3: {
    1: { height: 345, width: 625, x: 10, y: 10 },
    2: { height: 345, width: 625, x: 645, y: 10 },
    3: { height: 345, width: 625, x: 10, y: 365 }
  },
  4: {
    1: { height: 345, width: 625, x: 10, y: 10 },
    2: { height: 345, width: 625, x: 645, y: 10 },
    3: { height: 345, width: 625, x: 10, y: 365 },
    4: { height: 345, width: 625, x: 645, y: 365 }
  },
  5: {
    1: { height: 345, width: 413.33, x: 10, y: 10 },
    2: {
      height: 345,
      width: 413.33,
      x: 433.33,
    ...
    4: { height: 345, width: 413.33, x: 10, y: 365 },
    5: {
      height: 345,
      width: 413.33,
      x: 433.33,
  ...
  10: {
    1: { height: 226.67, width: 307.5, x: 10, y: 10 },
    2: { height: 226.67, width: 307.5, x: 327.5, y: 10 },
    3: { height: 226.67, width: 307.5, x: 645, y: 10 },
    4: { height: 226.67, width: 307.5, x: 962.5, y: 10 },
    5: {
      height: 226.67,
      width: 307.5,
      x: 10,
...
function addStreamsToState(state, streamsToAdd) {
  ...
  streamsToAdd.forEach((stream) => {
    if (stream.streamType === StreamType.AUDIO) {
      const trackExists = newState.audioStreams.find(
      ...
      const trackExists = newState.videoStreams.find(
        (existingStream) => existingStream.id === stream.id
      );
    ...
    for (const stream of newStreams) {
      if (stream.streamType === StreamType.AUDIO) {
        ...
        addVideoStreamToClient(
          newState.broadcastClient,
          stream,
          newState.videoStreams.length
        );
      }
    }
  }
...
function addCurrentParticipants(state) {
  ...
  for (let videoStream of state.videoStreams) {
    ...
    state.broadcastClient
      .addVideoInputDevice(
        new MediaStream([videoStream.mediaStreamTrack]),
        videoStream.id,
        {
          ...videoLayout,
          index: idx
...
function updateLayout(client, videos) {
  ...
  for (let video of videos) {
    ...
    client.updateVideoDeviceComposition(video.id, {
      ...participantLayout,
      index: idx
    });
    idx++;
...
export default function useBroadcast() {
  ...
  const startBroadcast = async () => {
    if (state.createError) {
      ...
      alert(
        'There was an error creating the broadcast client. Please check you have a valid ingest endpoint'
      );
      return;
    }
